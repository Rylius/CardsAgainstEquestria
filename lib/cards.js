    ...
    _.each(fs.readdirSync(directory), function (file) {
        ...
        _.each(data.blackCardsRaw, function (card) {
            ...
            card.toJSON = function () {
                return {id: card.id, text: card.text, watermark: data.watermark, draw: card.draw, pick: card.pick};
            };
...
var updateJson = function () {
    ...
    _.each(sets, function (cardSet) {
        cardSets.push({
            id: cardSet.id,
            name: cardSet.name,
            description: cardSet.description + ' ' + cardSet.blackCards.length + ' black cards, ' + cardSet.whiteCards.length + ' white cards.'
        });
    });
