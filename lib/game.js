...
/**
 * Converts a CahCreator card into the CAE format.
 * @author tjhorner
 */
...
class Player {
    ...
    toString() {
        return `[player ${this.user.id}/${this.user.name}]`
    }
...
class Game {
    ...
    removePlayer(user, reason) {
        ...
        if (this.czar == player) {
            if (this.waitingForCzar()) {
                ...
                this.czar = this.getNextCzar();

                this.czar.hand.push(...this.czar.move);
                this.czar.move = [];

                trace(this, `Selected ${this.czar} as the new czar`);
    ...
    startNextRound() {
        ...
        debug(this, 'Started round ' + this.round +
            ', czar ' + this.czar + ', black card ' + formatCard(this.blackCard));
    }
    ...
    onMove(user, cards) {
        ...
        if (this.waitingForCzar() || !this.blackCard) {
            trace(this, player + ' tried to play in a wrong state');
            return;
        }

        if (player.ignoreUntilNextRound) {
            trace(this, player + ' tried to play while being ignored');
            return;
        }

        if (player.move.length > 0) {
            trace(this, player + ' tried to play another move');
            return;
        }
        ...
        cards.forEach(id => {
            ...
            if (!card) {
                trace(this, player + ' tried to play invalid card ' + id);
                player.move = [];
                return;
            }
        ...
        if (player.move.length != this.blackCard.pick) {
            trace(this, player + ' tried to play an invalid move (played ' + player.move.length
                + ' cards, needed ' + this.blackCard.pick + ' cards)');
            player.move = [];
            return;
        }
        ...
        trace(this, player + ' made move: ' + JSON.stringify(player.move));
    ...
    roundState() {
        ...
        return {round: this.round, czar: this.czar.user.id, points: points, timeLeft: this.roundTimeLeft()};
    }

    roundTimeLeft() {
        if (this.roundTimeLimit) {
            return this.roundTimeLimit + Math.floor((this.timeLimitStart - new Date().getTime()) / 1000);
        } else {
            return 0;
        }
    }
    ...
    /**
     * Sends the complete game state for the given player.
     * @param {Player} player
     */
    sendState(player) {
        const user = player.user;

        trace(this, 'Sending state for ' + player);
