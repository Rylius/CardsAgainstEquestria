...
class Game {
    ...
    addPlayer(user) {
        ...
        if (this.state == constants.State.PLAYING) {
            ...
            var moveIds = _.collect(this.players, p => p.moveId);
            var id = moveIds[0];
    ...
    removePlayer(user, reason) {
        var player = _.find(this.players, p => p.user.id == user.id);
        if (!player) {
            return;
        }
    ...
    update(data) {
        ...
        if (data.sets != undefined) {
            this.sets.length = 0;
            _.each(data.sets, (s) => {
                var cardSet = _.find(cards.sets, cardSet => cardSet.id == s);
        ...
        if (data.expansions != undefined) {
            ...
            _.each(data.expansions, e => {
                var cardSet = _.find(cards.expansions, cardSet => cardSet.id == e);
    ...
    onSelect(user, data) {
        ...
        var movePlayer = _.find(this.players, p => p.moveId == selectedMove);
        if (!movePlayer) {
    ...
    sendUpdates() {
        ...
        _.each(this.players, player => {
            var request = this.updateRequests[player.user.id];
            var updates = this.updates[player.user.id];
    ...
    addBan(playerId, playerName, ip) {
        var existing = _.find(this.bans, bannedPlayer => bannedPlayer.id == playerId);
        if (!existing) {
