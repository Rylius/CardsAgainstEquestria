...
var Game = function (host) {
    ...
    this.removePlayer = function (user, reason) {
        ...
        } else if (this.waitingForCzar()) {
            this.invalidMoves.push(player.moveId);
        }
    ...
    this.onSelect = function (user, data) {
        ...
        if (!movePlayer) {
            if (!_.find(this.invalidMoves, function (invalid) {
                return selectedMove == invalid;
            })) {
            ...
            trace(this, 'Czar selected non-player move');
        }
...
exports.load = function (directory, config) {
    cards.loadSets(directory + '/' + config.files.sets);
};
